...
import org.meb.oneringdb.db.model.loc.Card_;
...
public class CardDao extends JpaDaoAbstract<Card, CardQuery> {
	...
	protected List<Predicate> buildCustomCriteria(CardQuery query, Root<Card> root) {
		...
		// Predicate prThisSignSquad = cb.equal(root.get(Card_.warlordId),
		// warlordId);
		// Predicate prFactions =
		// root.get(Card_.faction).in(faction.alliance());
		// Predicate prNotLoyal = cb.equal(root.get(Card_.loyal),
		// Boolean.FALSE);
		// Predicate prNotSignSquad = root.get(Card_.warlordId).isNull();
		//
		// predicates.add(prFactions);
		// predicates.add(root.get(Card_.type).in(CardType.TOKEN,
		// CardType.PLANET, CardType.WARLORD).not());
