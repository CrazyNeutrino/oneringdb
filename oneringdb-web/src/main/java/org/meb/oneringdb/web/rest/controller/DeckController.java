...
import org.meb.oneringdb.web.json.JsonDeckBuilder;
import org.meb.oneringdb.web.json.JsonUtils;
import org.meb.oneringdb.web.json.model.JsonDeck;
import org.meb.oneringdb.web.json.model.JsonDeckComment;
import org.meb.oneringdb.web.json.model.JsonDeckLink;
import org.meb.oneringdb.web.json.model.JsonDecks;
...
//			query.setSecondaryFactions(Faction.convertToFactions(params.getSecondaryFaction()));
	...
	public Response loadUserDeck(@PathParam("deckId") Long deckId, @QueryParam("language") String language)
		...
		try {
			...
			if (deck != null) {
				jsonDeck = new JsonDeckBuilder().withMembers().withLinks().withSnapshots().build(deck);
			}
	...
	public Response updateUserDeck(@PathParam("deckId") Long deckId, @QueryParam("language") String language,
		...
		try {
			...
			JsonDeck jsonDeck = new JsonDeckBuilder().withMembers().withLinks().withSnapshots().build(deck);
			// fillAllWarlordDeckCards(jsonDeck);
			return Response.ok(JsonUtils.write(jsonDeck)).build();
		} catch (Exception e) {
			DeckException de = buildDeckException(e, "error.deck.oper.save");
...
//			query.setSecondaryFactions(Faction.convertToFactions(params.getSecondaryFaction()));
			...
			JsonDecks jsonDecks = new JsonDecks();
			jsonDecks.setDecks(new JsonDeckBuilder().withMembers().withInterests().buildMany(decks));
			jsonDecks.setTotal(total);
			jsonDecks.setPageNumber(query.getPageNumber());
			jsonDecks.setPageSize(query.getPageSize());
	...
	public Response loadPublicDeck(@PathParam("deckId") String deckId, @QueryParam("language") String language) {
		...
		try {
			...
			JsonDeck jsonDeck = new JsonDeckBuilder().withMembers().withComments().withSnapshots().withInterests()
					.build(deck);
	...
	// HashMap<Long, JsonDeckMember> membersMap = new HashMap<Long,
	// JsonDeckMember>();
	// MapUtils.populateMap(membersMap, members, new Transformer<JsonDeckMember,
	// Long>() {
	//
	// @Override
	...
	// });
	//
	// List<Card> warlordDeckCards =
	// ccm.getWarlordCards(jsonDeck.getWarlordId());
