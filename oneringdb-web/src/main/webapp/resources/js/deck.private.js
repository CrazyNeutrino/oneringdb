...
(function(_deck) {
	...
	_deck.showDeckPublishModal = function(deck, options) {
		...
		var publish = function($modal, name, description) {
			var attributes = {
				...
				snapshotPublic : true,
				tournamentType : $modal.find(
						'.btn-group.tournament-type > .btn.active').data(
						'tournament-type'),
				tournamentPlace : $modal.find(
						'.btn-group.tournament-place > .btn.active').data(
						'tournament-place'),
			};
	...
	_deck.ViewBase = Backbone.View.extend({
		...
		viewLinkClickHandler : function(event) {
			...
			if (href && href.indexOf(root) == 0 && !event.ctrlKey
					&& !event.shiftKey) {
	...
	_deck.UserDeckListView = _deck.ViewBase.extend({
		...
		render : function() {
			...
			_.each([
					...
					[ 'attachmentCardsQuantity',
							'core.cardsQuantity.attachment' ],
			...
			view.deckListFilterView.state.on('change:advanced',
					function(state) {
	 ...
	 * Deck edit view
	 */
	_deck.UserDeckEditView = _deck.ViewBase
			.extend({
				events : {
				...
				onSelectOneGroupClick : function(event) {
					...
					$(event.currentTarget).addClass('active').siblings()
							.removeClass('active');
				},
				...
				onSphereFilterClick : function(e) {
					...
					this.membersFilter.set({
						sphere : $(e.currentTarget).parent().children().filter(
								'.active').map(function() {
				...
				onCardTypeFilterClick : function(e) {
					...
					this.membersFilter.set({
						type : $(e.currentTarget).parent().children().filter(
								'.active').map(function() {
				...
				onSelectionFilterClick : function(e) {
					...
					this.membersFilter.set({
						quantity : _.flatten($(e.currentTarget).parent()
								.children().filter('.active').map(function() {
				...
				applyFilterToUI : function() {
					var filter = _.pick(this.membersFilter.attributes,
							ordb.filter.CARD_ATTRS);
					...
					this.$el.find('.filter-group.filter-sphere .btn').each(
							function() {
								var $this = $(this);
								if (_.contains(filter.faction, $this
										.data('sphere'))) {
									$this.addClass('active');
								}
							});
					this.$el.find('.filter-group.filter-card-type .btn').each(
							function() {
								var $this = $(this);
								if (_.contains(filter.type, $this
										.data('card-type'))) {
									$this.addClass('active');
								}
							});
					this.$el
							.find('.filter-group.filter-selection .btn')
							.each(
									function() {
										var $this = $(this);
										if (_
												.contains(
														filter.quantity,
														$this.data('selection') == 'non-selected' ? 0
																: 1)) {
											$this.addClass('active');
										}
									});
				...
				renderPrivateLinksList : function() {
					...
					//
					this.$el
							.find('#deckPrivateLinkList .btn')
							.click(
									function() {
										var linkId = parseInt($(this).closest(
												'tr').data('id'));
				...
				renderPublishedDecksList : function() {
					...
					this.$el.find(
							'#deckPublishedDecksBlock [data-toggle="tooltip"]')
							.tooltip({
								container : 'body'
							});
					...
					// edit published deck
					//
					this.$el.find('.deck-oper-edit').click(
							function() {
								showDeckEditSnapshotModal(this.deck.get(
										'snapshots').findWhere(
										{
											id : parseInt($(this).closest('tr')
													.data('id'))
										}), {
					...
					//				
					this.$el
							.find('.deck-oper-delete')
							.click(
									function() {
										var deckId = parseInt($(this).closest(
												'tr').data('id'));
										var deleteHandler = function() {
											this.deck
													.get('snapshots')
				...
				filterMembers : function() {
					...
					var filteredMembers = this.membersFilter.filter(this.deck
							.get('members'));
					this.deck.get('filteredMembers').comparator = this
							.buildMembersComparator(this.membersSorter
									.get('keys'));
				...
				sortMembers : function() {
					...
					this.deck.get('filteredMembers').trigger('reset',
							this.deck.get('filteredMembers'));
				},
				...
				updateStats : function() {
					...
					this.$el.find(
							'.deck-stats-container [data-toggle="tooltip"]')
							.tooltip({
								container : 'body',
								trigger : 'hover click'
							});
				...
				render : function(options) {
					...
					var renderInternal = function() {
						...
						view.membersFilter.on('change', view.filterMembers,
								view);
						...
						// content: function() {
						// return
						// ordb.ui.writeCardImgElem($(this).data('image-base'),
						// {
						// class: 'card-md'
						// });
						// }
						// // template: '<div class="popover popover-card"
						// role="tooltip"><div class="arrow"></div><h3
						// class="popover-title"></h3><div
						// class="popover-content"></div></div>',
						// // content:
						// Handlebars.template['card-text-content']()
						// });
						// var $popovers =
						// view.$el.find('a[data-card-id]').popover({
						// html: true,
						// trigger: 'hover',
						// template: '<div class="popover popover-card"
						// role="tooltip"><div class="arrow"></div><h3
						...
						// content: function() {
						// return
						// Handlebars.templates['card-text-content'](ordb.dict.findCard($(this).data('card-id')));
						// }
						// });
						...
						view.membersListView.listenTo(view.deck
								.get('filteredMembers'), 'reset', function(
								filteredMembers) {
													...
													.listenTo(
															...
															function(member,
																...
																if (options.batchChange !== true) {
																	...
																	var $members = view.membersListView.$el
																			.find('.members-list-item, .members-grid-item');
																	var $buttons = $members
																			.filter(
																					'[data-card-id="'
																							+ member
						...
						view.listenTo(view.config, 'change:layout', function(
								config) {
							view.membersListView.render(view.deck
									.get('filteredMembers'), {
						...
						view.listenTo(view.deck, 'change:configCsQuantity',
								function(deck) {
									...
									view.groupsView.render(view.deck
											.get('members'), {
										readOnly : false
									});
									view.updateStats();
						...
						$(
								'.btn-group.layout-group > .btn[data-layout="'
										+ view.config.get('layout') + '"]')
								.addClass('active');

						//
						// save deck
						//
						$('.btn.deck-save, a.deck-save')
								.click(
										function() {
											var attributes = {
												name : view.deckDescriptionView.$el
														.find('#deckName')
														.val().trim(),
												description : view.deckDescriptionView.$el
														.find(
																'#deckDescription')
														.val().trim()
											};
											var deckIdBeforeSave = view.deck
													.get('id');
											view.deck
													.save(
															attributes,
												...
												view.deck
														.destroy({
															...
															success : function(
																...
																ga('send',
																		'pageview');
																userDeckListView.messages = _deck
																		.buildSuccessMessage({
																			message : 'ok.deck.oper.delete'
																		});
																delete view.deck;
						...
						view.listenTo(view.deck.get('snapshots'), 'add remove',
								view.renderPublishedDecksList);
						...
						view.listenTo(view.deck.get('links'), 'add remove',
								view.renderPrivateLinksList);
						...
						view.$el
								.find(
										'.members-list-filter-container[data-toggle="tooltip"]')
								.tooltip({
									container : 'body'
								});
						...
						$configTrigger
								.on(
										'shown.bs.popover',
										function() {
											var $configContent = view.$el
													.find('#configContent');
											var $configCsQuantityRadios = $configContent
													.find('input[name="csQuantity"]');
											$configCsQuantityRadios
													.filter(
															'[value="'
																	+ view.deck
																			.get('configCsQuantity')
																	+ '"]')
													.prop('checked', true);
											$configContent
													.find('#configApply')
													.click(
															function() {
																...
																view.deck
																		.set({
																			configCsQuantity : parseInt($configCsQuantityRadios
																					.filter(
																							':checked')
																					.val())
																		});
								...
								.click(function() {
									...
									var draw = function(quantity) {
										if (_.isUndefined(view.shuffledCards)) {
											view.shuffledCards = ordb.util
													.membersShuffle(view.deck
															.get('members')
															.filter(
																	function(
																			member) {
										...
										var drawn = 0;
										while (view.shuffledCardsIndex < view.shuffledCards.length
												&& drawn < quantity) {
						...
						//
						var $typeahead = ordb.ui.buildCardsTypeahead(
								view.membersFilter, {
	...
	_deck.UserDeckImportView = _deck.ViewBase
			.extend({
				events : {
							...
							.click(
									function() {
														...
														function(line) {
																	...
																	.some(
																			...
																			function(
																				...
																				var tmp = s(
																						...
																						.toLowerCase();
																				return tmp
																						.startsWith(cardType.name
																								.toLowerCase())
																						|| tmp
																								.startsWith(cardType.nameEn
																										.toLowerCase());
																			});
															...
															if (tokens != null) {
																...
																if (_
																		.isUndefined(card)) {
																	warnings
																			.push(ordb.dict.messages['core.skipped']
																					+ ': '
																					+ line);
																	return;
																}
										...
										if (_.isUndefined(warlordId)) {
											errors
													.push(ordb.dict.messages['error.deck.warlord.notFound']);
										} else {
											var pdms = _deck
													.getPlayerDeckMembers();
											var pdmsIndex = _.indexBy(pdms,
													function(pdm) {
														return pdm.cardId;
													});
											_
													.each(
															...
															function(member) {
																...
																if (_
																		.isUndefined(pdm)) {
																	errors
																			.push(ordb.dict.messages['error.deck.invalidCard']
																					+ ': '
																					+ ordb.dict
																							.findCard(member.cardId).name);
																} else {
																	var card = ordb.dict
																			.findCard(member.cardId);
																	if (_
																			.isNumber(card.warlordId)) {
																		pdm.quantity = card.quantity;
																	} else if (_
																			.isNumber(member.quantity)) {
																		pdm.quantity = Math
																				.max(
																						1,
																						Math
										...
										if (errors.length > 0) {
											$container
													.append(Handlebars.templates['commons-ul']
															({
